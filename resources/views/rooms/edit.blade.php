...
@section('title', 'Edit Room - hypercare')
...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            ...
            <h1 class="text-3xl font-black tracking-tight text-text">Edit Room</h1>
            <p class="text-sm text-text-faint font-medium mt-1">Update room details</p>
        </div>
    ...
    <div class="max-w-2xl">
        <x-ui.card padding="none">
            <div class="p-10">
                <form method="POST" action="{{ route('rooms.update', $room) }}" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-3">
                            ...
                            <select id="room_class" name="room_class" required class="w-full px-6 py-4.5 bg-white border border-border/60 rounded-2xl text-text font-bold focus:ring-4 focus:ring-accent/10 focus:border-accent/40 transition-all outline-none text-sm">
                                ...
                                <option value="VIP" {{ old('room_class', $room->room_class) == 'VIP' ? 'selected' : '' }}>VIP</option>
                                <option value="Class I" {{ old('room_class', $room->room_class) == 'Class I' ? 'selected' : '' }}>Class I</option>
                                <option value="Class II" {{ old('room_class', $room->room_class) == 'Class II' ? 'selected' : '' }}>Class II</option>
                                <option value="Class III" {{ old('room_class', $room->room_class) == 'Class III' ? 'selected' : '' }}>Class III</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="number" id="price_per_night" name="price_per_night" value="{{ old('price_per_night', $room->price_per_night) }}" required min="0"
                                class="w-full px-6 py-4.5 bg-white border border-border/60 rounded-2xl text-text font-bold placeholder-text-faint/50 focus:ring-4 focus:ring-accent/10 focus:border-accent/40 transition-all outline-none text-sm">
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $room->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded text-accent focus:ring-accent/20">
