...
@section('title', 'Doctors Master Data - hypercare')
...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            ...
            <h1 class="text-3xl font-black tracking-tight text-text">Doctors</h1>
            <p class="text-sm text-text-faint font-medium mt-1">Manage doctor profiles and specializations</p>
        </div>
        <div class="flex items-center gap-3">
            <x-ui.button href="{{ route('doctors.create') }}" variant="primary" size="lg" class="shadow-xl shadow-accent/20">
    ...
    <x-ui.card class="mb-8" padding="none">
            ...
            <table class="w-full text-left border-separate border-spacing-y-3">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 pb-4 text-[0.6rem] font-bold text-text-faint uppercase tracking-[0.2em]">User Account</th>
                        <th class="px-6 pb-4 text-[0.6rem] font-bold text-text-faint uppercase tracking-[0.2em]">Specialization</th>
                        <th class="px-6 pb-4 text-[0.6rem] font-bold text-text-faint uppercase tracking-[0.2em]">Phone</th>
                    ...
                    @forelse ($doctors as $doctor)
                        <tr class="group hover:bg-bg/40 transition-all duration-300">
                            <td class="px-6 py-5 bg-white group-hover:bg-bg/10 rounded-l-2xl border-y border-l border-border/40 group-hover:border-accent/20 transition-all">
                                <p class="text-sm font-bold text-text leading-tight mb-1 group-hover:text-accent transition-colors">{{ $doctor->name }}</p>
                            </td>
                            <td class="px-6 py-5 bg-white border-y border-border/40 group-hover:bg-bg/10 group-hover:border-accent/20 transition-all">
                                <span class="text-xs font-medium text-text-muted">{{ $doctor->user->name ?? '---' }}</span>
                            </td>
                            <td class="px-6 py-5 bg-white border-y border-border/40 group-hover:bg-bg/10 group-hover:border-accent/20 transition-all">
                                <span class="text-xs font-medium text-text-muted">{{ $doctor->specialization ?? '---' }}</span>
                            </td>
                            <td class="px-6 py-5 bg-white border-y border-border/40 group-hover:bg-bg/10 group-hover:border-accent/20 transition-all">
                                <span class="text-xs font-medium text-text-muted">{{ $doctor->phone ?? '---' }}</span>
                            </td>
                            <td class="px-6 py-5 bg-white border-y border-border/40 group-hover:bg-bg/10 group-hover:border-accent/20 text-center transition-all">
                                <x-ui.badge variant="{{ $doctor->is_active ? 'success' : 'danger' }}">
                            ...
                            <td class="px-6 py-5 bg-white border-y border-r border-border/40 group-hover:bg-bg/10 group-hover:border-accent/20 rounded-r-2xl text-right transition-all">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                    <x-ui.button href="{{ route('doctors.edit', $doctor) }}" variant="secondary" size="xs" class="w-9 h-9 !p-0 flex items-center justify-center">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                        </svg>
                                    </x-ui.button>
                                    <form method="POST" action="{{ route('doctors.destroy', $doctor) }}" class="inline-block" onsubmit="return confirm('Delete this doctor?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-24 text-center">
                                <div class="flex flex-col items-center max-w-sm mx-auto">
                                    <h3 class="text-xl font-black text-text mb-2">No Doctors Found</h3>
                                    <p class="text-sm text-text-faint font-medium mb-10 leading-relaxed text-balance">Get started by creating a new doctor.</p>
                                    <x-ui.button href="{{ route('doctors.create') }}" variant="primary" size="md" class="px-8 shadow-lg shadow-accent/20">
                                        Add Doctor
                                    </x-ui.button>
                                </div>
                            </td>
        ...
        @if ($doctors->hasPages())
            <div class="px-8 py-8 border-t border-border/40 bg-bg/10 rounded-b-[2.5rem]">
                {{ $doctors->links('pagination.simple') }}
            </div>
        @endif
    </x-ui.card>
@endsection
