...
@section('title', 'Edit Doctor - hypercare')
...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            ...
            <h1 class="text-3xl font-black tracking-tight text-text">Edit Doctor</h1>
            <p class="text-sm text-text-faint font-medium mt-1">Update doctor details</p>
        </div>
    ...
    <div class="max-w-2xl">
        <x-ui.card padding="none">
            <div class="p-10">
                <form method="POST" action="{{ route('doctors.update', $doctor) }}" class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $doctor->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded text-accent focus:ring-accent/20">
