...
@section('title', 'Edit Doctor Schedule - hypercare')
...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            ...
            <h1 class="text-3xl font-black tracking-tight text-text">Edit Schedule</h1>
        </div>
    ...
    <form method="POST" action="{{ route('doctor-schedules.update', $doctorSchedule) }}">
        @csrf
        @method('PUT')
                ...
                <x-ui.card>
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-text-faint font-medium">Update details for the selected schedule.</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="relative">
                                    ...
                                    <select id="day_of_week" name="day_of_week" required class="block w-full pl-11 pr-10 py-4 bg-bg border-2 border-border/50 hover:border-accent/50 focus:border-accent focus:ring-0 rounded-2xl text-sm font-bold text-text transition-all appearance-none cursor-pointer">
                                        ...
                                        @foreach($days as $index => $day)
                                            <option value="{{ $index }}" {{ (old('day_of_week') ?? $doctorSchedule->day_of_week) == $index ? 'selected' : '' }}>
                                                {{ $day }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" id="quota" name="quota" value="{{ old('quota', $doctorSchedule->quota) }}" required min="1"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="time" id="start_time" name="start_time" value="{{ old('start_time', \Carbon\Carbon::parse($doctorSchedule->start_time)->format('H:i')) }}" required 
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="time" id="end_time" name="end_time" value="{{ old('end_time', \Carbon\Carbon::parse($doctorSchedule->end_time)->format('H:i')) }}" required 
                                    class="block w-full px-4 py-4 bg-bg border-2 border-border/50 hover:border-accent/50 focus:border-accent focus:ring-0 rounded-2xl text-sm font-bold text-text transition-all">
                                @error('end_time')
                                    <p class="text-xs font-bold text-red-500 mt-2 flex items-center gap-1">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                        ...
                        <div class="flex items-center gap-3 mt-4">
                            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $doctorSchedule->is_active) ? 'checked' : '' }} class="w-5 h-5 text-accent border-2 border-border/50 rounded-lg focus:ring-accent focus:ring-offset-bg bg-bg transition-colors cursor-pointer">
