...
@section('title', 'Edit Patient Record - hypercare')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            <div class="flex items-center gap-2 mb-2">
                ...
                <p class="text-accent text-[0.7rem] font-black tracking-[0.2em] uppercase">Clinical Registry</p>
            </div>
            <h1 class="text-3xl font-black tracking-tight text-text">Edit Patient Record</h1>
            <p class="text-sm text-text-faint font-medium mt-1">Modify clinical and identity information for patient: <span class="text-text font-bold">{{ $patient->name }}</span></p>
        </div>
    ...
    <div class="max-w-4xl mx-auto">
        <x-ui.card padding="none">
            <div class="p-10">
                @if ($errors->any())
                    <div class="mb-8 p-6 bg-danger/5 border border-danger/10 rounded-2xl animate-fade-in">
                        <div class="flex items-center mb-4">
                            ...
                            <span class="font-black text-danger uppercase tracking-widest text-xs">Validation Error</span>
                        </div>
                ...
                <form method="POST" action="{{ route('patients.update', $patient) }}" class="space-y-12">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        <div class="flex items-center gap-4 mb-10">
                            ...
                            <div>
                                ...
                                <p class="text-[0.65rem] text-text-faint font-bold uppercase tracking-wider">Update clinical and identity details</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                            <div class="space-y-3">
                                ...
                                <div class="relative group">
                                    <input type="text" id="medical_record_number" value="{{ $patient->medical_record_number }}" disabled 
                                    ...
                                    <div class="absolute right-4 top-1/2 -translate-y-1/2 px-2.5 py-1.5 bg-white border border-border/60 rounded-lg text-[0.55rem] font-black uppercase text-accent/60 tracking-widest shadow-sm">Immutable MRN</div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative group">
                                    <select id="gender" name="gender" required
                                        ...
                                        <option value="Laki-laki" {{ old('gender', $patient->gender) == 'Laki-laki' ? 'selected' : '' }}>Male</option>
                                        <option value="Perempuan" {{ old('gender', $patient->gender) == 'Perempuan' ? 'selected' : '' }}>Female</option>
                                    </select>
                                ...
                                <div class="relative group">
                                    <select id="blood_type" name="blood_type"
                                        ...
                                        @foreach(['A', 'B', 'AB', 'O', '-'] as $bt)
                                            <option value="{{ $bt }}" {{ old('blood_type', $patient->blood_type) == $bt ? 'selected' : '' }}>{{ $bt == '-' ? 'Not Specified' : $bt }}</option>
                                        @endforeach
                                    </select>
                                    <div class="absolute inset-y-0 right-0 flex items-center px-5 pointer-events-none text-text-faint group-hover:text-accent transition-colors">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative group">
                                    <select id="insurance_type" name="insurance_type" required
                                        class="appearance-none w-full px-6 py-4.5 bg-white border border-border/60 rounded-2xl text-text font-bold focus:ring-4 focus:ring-accent/10 focus:border-accent/40 transition-all outline-none cursor-pointer text-sm">
                                        <option value="Umum" {{ old('insurance_type', $patient->insurance_type) == 'Umum' ? 'selected' : '' }}>General Private (Umum)</option>
                                        <option value="BPJS" {{ old('insurance_type', $patient->insurance_type) == 'BPJS' ? 'selected' : '' }}>Social Health (BPJS)</option>
                                        <option value="Asuransi" {{ old('insurance_type', $patient->insurance_type) == 'Asuransi' ? 'selected' : '' }}>Corporate Insurance</option>
                                    </select>
                                    <div class="absolute inset-y-0 right-0 flex items-center px-5 pointer-events-none text-text-faint group-hover:text-accent transition-colors">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>
                                    </div>
                            ...
                            <div class="space-y-3 md:col-span-2">
                                ...
                                <textarea id="address" name="address" rows="3" 
                                    ...
                                    class="w-full px-6 py-5 bg-white border border-border/60 rounded-2xl text-text font-bold placeholder-text-faint/50 focus:ring-4 focus:ring-accent/10 focus:border-accent/40 transition-all outline-none resize-none min-h-[140px] text-sm leading-relaxed">{{ old('address', $patient->address) }}</textarea>
                            </div>
                        </div>
                    </div>
