...
@section('title', 'Edit User - Hypercare')
...
<div class="mb-6 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-text">Edit User</h1>
        <p class="text-sm text-text-muted mt-1">Update user account information.</p>
    </div>
</div>

<div class="max-w-3xl">
    <form action="{{ route('users.update', $user) }}" method="POST" class="bg-surface border border-border/60 rounded-2xl shadow-sm p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <div class="relative">
                    <select id="role" name="role" required class="w-full px-4 py-2.5 bg-bg border @error('role') border-danger @else border-border/60 @enderror rounded-xl text-sm focus:outline-none focus:ring-4 focus:ring-accent/10 focus:border-accent/40 transition-all appearance-none">
                        ...
                        <option value="nurse" @selected(old('role', $user->role) == 'nurse')>Nurse</option>
            ...
            <div class="pt-4 border-t border-border/60">
                <h3 class="text-sm font-bold text-text mb-3">Change Password <span class="font-normal text-text-muted text-xs">(leave blank to keep current)</span></h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="password" class="block text-sm font-semibold text-text mb-1.5">New Password</label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-semibold text-text mb-1.5">Confirm New Password</label>
