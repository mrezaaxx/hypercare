...
@section('title', 'Laboratory Registry - hypercare')
...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            <div class="flex items-center gap-2 mb-2">
                ...
                <p class="text-info text-[0.7rem] font-black tracking-[0.2em] uppercase">Diagnostic Services</p>
            </div>
            <h1 class="text-3xl font-black tracking-tight text-text">Lab Order Registry</h1>
            <p class="text-sm text-text-faint font-medium mt-1">Monitor and manage clinical laboratory requests</p>
        </div>
        <div class="flex items-center gap-3">
            <x-ui.button href="{{ route('lab-orders.create') }}" variant="primary" size="md">
    ...
    <x-ui.card padding="none" class="mb-8 overflow-hidden">
        ...
        <div class="p-8 border-b border-border/40 bg-bg/20 backdrop-blur-sm">
            <form method="GET" action="{{ route('lab-orders.index') }}" class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @foreach ($filters as $filter)
                        <a href="{{ route('lab-orders.index', $filter['status'] ? ['status' => $filter['status']] : []) }}" 
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-bg/10">
                        ...
                        <th class="px-8 py-5 text-[0.6rem] font-black text-text-faint uppercase tracking-[0.2em]">Clinical Test</th>
                        <th class="px-8 py-5 text-[0.6rem] font-black text-text-faint uppercase tracking-[0.2em]">Priority</th>
                        <th class="px-8 py-5 text-[0.6rem] font-black text-text-faint uppercase tracking-[0.2em]">Current Status</th>
                ...
                <tbody class="divide-y divide-border/40">
                    @forelse ($orders as $order)
                        <tr class="group hover:bg-bg/30 transition-all duration-300">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-text-muted">{{ $order->test_type }}</span>
                                    <span class="text-[0.6rem] text-text-faint font-black uppercase tracking-widest mt-1">Diagnostic</span>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                    <x-ui.button href="{{ route('lab-orders.edit', $order) }}" variant="secondary" size="sm" class="!p-2.5 !rounded-xl" title="Edit Order">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                        </svg>
                                    </x-ui.button>
                                    <form method="POST" action="{{ route('lab-orders.destroy', $order) }}" class="inline-block" onsubmit="return confirm('Delete this laboratory order?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-8 py-24 text-center">
                                <div class="flex flex-col items-center max-w-sm mx-auto">
                                    ...
                                    <h3 class="text-xl font-black text-text mb-3">No Lab Orders Found</h3>
                                    <p class="text-sm text-text-faint font-bold leading-relaxed mb-10">The laboratory registry is currently empty. Initiate diagnostic workflows by creating a new order.</p>
                                    <x-ui.button href="{{ route('lab-orders.create') }}" variant="primary" size="lg" class="shadow-xl shadow-info/20">
